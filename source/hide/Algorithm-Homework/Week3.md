...
将 v 分为 $\lceil \frac{n}{m} \rceil$ 段，每段长度最多为 $m$。计算 $ u v $ 需要 $\lceil \frac{n}{m} \rceil $ 次 $ m \times m $ 位的乘法，每次乘法可以用题述分治法在 $ O(m^{\log 3}) $ 的时间内处理完毕。总时间复杂度为 $ O(\frac{n}{m} m^{\log 3}) = O(n m^{\log \frac{3}{2}}) $。
...
先将 $ n $ 阶矩阵分为 $ m \times m $ 个 $ 2^k $ 阶子矩阵，传统方法处理 $ m \times m $ 分块矩阵需要 $ O(m^3) $ 的合并时间；Strassen 算法处理 $ 2^k $ 阶矩阵相乘需要 $ O(7^k) $ 的时间。
...
很明显 $n_i$ 是所求多项式的零点，根据零点可以直接构造一个多项式：

$$
\begin{align}
P(x) & = \prod_{i = 1}^{d}{(x - n_i)} \\
     & = \begin{cases}
		     \prod_{i = 1}^{\frac{d}{2}}{(x - n_i)} \prod_{i = \frac{d}{2} + 1}^{d}{(x - n_i)} & d \equiv 0 \mod 2 \\
		     \prod_{i = 1}^{d - 1}{(x - n_i)} (x - n_d) & d \equiv 1 \mod 2 \\
		 \end{cases}
\end{align}
$$

$ d $ 为偶数时，可以直接等长拆分成两部分，套用题设中第二种算法需要 $ O(d \log d) $ 的合并时间。

$ d $ 为奇数时，可以拆分成 $ d - 1 $ 次多项式与 $ 1 $次多项式这两部分，套用题设中第一种算法需要 $ O(d) $ 的合并时间。

> 另外当$ d $为奇数时，也可以直接补一个 $ n_{d + 1} = 0 $ 来使得问题可以均等拆分，对最终复杂度没有影响。
...
T(d) = \begin{cases}
    O(1) & d = 1 \\
    2T(\frac{d}{2}) + O(d \log d) & d \equiv 0 \mod 2 \\
		T(d - 1) + O(d) & \text{otherwise} \\
\end{cases}
$$
...
这实际上是一个经典的**平方分治**结合**多路归并排序**的方法。

分治的部分很好理解：将 $ n $ 个元素的序列划分为 $ k = \lfloor \sqrt{n} \rfloor $ 个子序列，总长度为 $ n $，递归执行这个归并排序得到 $ k $ 个有序子序列。

合并这些子序列的算法是有一些技巧的：

将 $ k $ 个有序子序列看作一个队列，**每次取出** $ k $ 个子序列的队首中最小的元素作为最终有序序列的下一个元素，即可完成归并。

但是如何在 $ k $ 个序列中找到最小的元素呢？

线性的比较算法：每次遍历这 $ k $ 个队列的队首，维护一个最小值的位置。需要 $ O(k) $ 的时间来完成单次取数，总共有 $ n $ 次取数操作。那么需要 $ O(k n) = O(n \sqrt{n}) $ 的时间来完成这个归并。
...
$$
T(n) = O(\sum_{k = 0}^{\lfloor \log_2 n \rfloor + 1 }{n^{\frac{3}{2^{2k+1}}}})
$$

这个复杂度肯定是不小于 $O(n \sqrt{n})$ 的。

这个答案虽然比较遗憾，但是也是符合题设要求的。

不过可以利用一个优先队列(堆)来加速这个过程：

将 $ k $ 个队列的队首取出放入优先队列，保持这个优先队列中最多有 $ k $ 个元素，然后从优先队列中取出队首即为总序列的下一个元素，这需要 $ O(\log k) $ 的时间，然后再将这个元素对应的子队列的新队首取出放入优先队列，这需要 $ O(1) $ 的时间。总共有 $ n $ 次取数操作，那么需要 $ O(n \log k) = O(n \log n)$ 的时间完成这个归并。

$$
T(n) = \sqrt{n} T(\sqrt{n}) + O(n \log n)
$$
...
在划分(Partition)阶段有一个二元比较关系$ R $，如果这个关系满足定义：

$$
(x, y) \in R \Leftrightarrow x \ge y
$$

则快速排序算法给出非增序排序。

> 更广义地来说，只要这个关系是一个严格弱序关系(满足反自反性、反对称性、传递性、连通性)，快速排序就能保证停机性与正确性。排序算法是一个可配置的算法。

{% include_code "可配置的快速排序" Algorithm-Homework-Reference/A-2-13.cpp lang:cpp %}

2-14 Gray码是一个长度为 $ 2^n $ 的序列。序列中无相同元素，每个元素都是长度为 n 位的 (0, 1) 串，相邻元素恰好只有一位不同。用分治策略设计一个算法对任意的 n 构造相应的 Gray 码。

格雷码本质上也是一种编码，它与自然二进制数存在一个双射，这一点通过抽屉原理很容易证明。

设 $ Gray: [0, 2^n) \mapsto [0, 2^n) $ 为这个双射函数，如 $ Gray(4) = 6 = (0\cdots0110)_2 $ 。
...
Gray(n) = \begin{cases}
  0 & n = 0 \\
  2^{\lfloor \log_2{n} \rfloor} + Gray(2^{\lceil \log_2{n + 1} \rceil} - 1 - n) & n > 0 \\ 
\end{cases}
$$
...
有了这个递归式直接将 Gray 函数的返回值以n位自然二进制格式输出即可。

{% include_code "Gray Code 记忆化" Algorithm-Homework-Reference/A-2-14.cpp lang:cpp %}
...
> 但如果只是求解排列的总数，可能再对某数取模，还是比较简单的，利用多重组合数公式+记忆化+乘法同余式就能轻松解决。
